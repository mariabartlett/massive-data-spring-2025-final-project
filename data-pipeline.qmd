...
We processed the EADA datasets locally and uploaded `Schools.csv` and `InstLevel.csv` to an S3 bucket. We selected Amazon S3 as our data storage due to its security features and integration with AWS analytical tools. This approach allows us to centralize multi-year EADA datasets. 
